 ...
 */
{
  "hitLog": {
    ...
    "hpLeft": {
        ...
        "format": "<font size='12'><textformat leading='-4' tabstops='[26,60,135]'><font color='{{c:hp-ratio}}'>{{hp}}</font><tab><font color='#FFFFFF'> / </font>{{hp-max}}<tab><font color='#FFFFFF'> | </font><font color='{{c:vtype}}'>{{vehicle}}</font><tab><font color='#FFFFFF'>|<font>{{name}}</font> <font color='#CC99'>{{clan}}</font></font></textformat></font>"
    },
    ...
    "deadMarker": "<font size='15'><font face='Wingdings'>N</font></font>",
    "blowupMarker": "<font size='15'><font face='Wingdings'>M</font></font>",
    ...
    "defaultHeader":  "<font color='#FF000'>{{l10n:Hits}}:</font> <font size='13'>#0</font>",
    ...
    "formatHeader": "<textformat leading='-3'>{{l10n:Hits}}: <font color='#FF000'><font size='13'>#{{n}}</font></font> <b>{{l10n:Total}}: <font color='#02C9B3'>{{dmg-total}}</b></font>  <font color='#FFFFFF'>{{l10n:Last}} попадание:</font> <font color='{{c:dmg-kind}}'><b>{{dmg}}</b> {{dead}}</font></textformat>",
    // List of hits format (macros allowed, see readme-en.txt).
    // Формат лога попаданий (допускаются макроподстановки, см. readme-ru.txt)
    "formatHistory": "<textformat leading='-3' tabstops='[14,55,95,145]'><font face='Consolas' size='11'><font>\u00D7{{n-player}}:<tab>| </font><font color='#FFB964'>{{dmg-player}}</font><tab>| {{dmg}}<tab>| <font color='{{c:dmg-kind}}'>{{dmg-kind}}</font><tab>| <font color='{{c:vtype}}'>{{vehicle}}</font><font color=''> ({{name}}<font color='#CC99'>{{clan}}</font>)</font> <font color='{{c:dmg-kind}}'><b>{{dead}}</b></font></font></textformat>",
    // Shadow options.
    // Параметры тени.
    "shadow": {
      // Opacity.
