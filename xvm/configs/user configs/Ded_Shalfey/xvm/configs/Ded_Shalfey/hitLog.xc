 ...
 */
{
  "hitLog": {
    ...
    "formatHeader":  "<font color='#FFFFFF'>{{l10n:Total}}:</font> <font size='14'></font> <font size='14'><b><font color='#FFFF00'><u>{{dmg-total}}</u> хп</font></b> <font color='#FFFFFF'>{{l10n:Hits}}:</font> <b><font color='#FFFF00'>{{n}}</font></b>\n<font color='#FFFFFF'>{{l10n:Last}}:</font> <font color='#FFFF00'><b>{{dmg}} хп</b> </font><font color='{{c:vtype}}'>({{vehicle}} {{dead}})</font></font>",
    // List of hits format (macros allowed, see readme-en.txt).
    // Формат лога попаданий (допускаются макроподстановки, см. readme-ru.txt)
    "formatHistory": "<textformat leading='-1' tabstops='[20,50,90,150]'><font size='12' color='#FFFFFF' face='Comic Sans MS'>\u00D7{{n-player}}:</font><tab><font color='#FFFFFF'>{{dmg-player}}</font><tab>| <font color='{{c:dmg-kind}}'>{{dmg}}</font><tab>| <font color='{{c:dmg-kind}}'>{{dmg-kind}}</font><tab>| <font color='{{c:vtype}}'>{{vehicle}} {{dead}}</font></textformat>",
    // Shadow options.
    // Параметры тени.
    "shadow": {
      // Opacity.
